    ...
    <div className="min-h-screen bg-bg flex items-center justify-center p-6">
      <div className="w-full max-w-[360px] bg-surface border border-border rounded-md shadow-e2 p-6">
        ...
        <form onSubmit={handle} className="grid gap-4">
          <div className="grid gap-1.5"><label className="text-[11px] font-bold uppercase tracking-[0.08em] text-text-2">Name</label><input value={name} onChange={(e) => setName(e.target.value)} className="w-full min-h-[46px] p-3 border border-border bg-surface-2 rounded-sm text-[13.5px] focus:border-brand focus:bg-surface outline-none" /></div>
          ...
          <button type="submit" className="min-h-[44px] bg-brand text-on-brand rounded-sm font-semibold hover:bg-brand-hover transition-colors">Create account</button>
        </form>
